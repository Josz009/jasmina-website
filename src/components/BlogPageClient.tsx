...
export default function BlogPageClient({ featured, articles }: Props) {
  ...
  return (
    <div ref={pageRef}>
      ...
      <section style={{ position: "relative", minHeight: "55vh", display: "flex", alignItems: "center", justifyContent: "center", textAlign: "center", overflow: "hidden" }}>
        <Image src="/images/stock/blog-header.jpg" alt="Blog & Articles" fill priority style={{ objectFit: "cover" }} sizes="100vw" />
        ...
        <div className="blog-hero-content" style={{ position: "relative", zIndex: 2, maxWidth: 700, padding: "130px 32px 90px" }}>
          <span className="sec-tag sec-tag-gold" style={{ marginBottom: 16 }}>blog & articles</span>
          <h1 style={{ fontFamily: "var(--font-display)", fontSize: "3.4rem", fontWeight: 400, color: "#fff", lineHeight: 1.15, marginBottom: 20, textShadow: "0 2px 24px rgba(0,0,0,0.4)" }}>
            Financial <em style={{ fontStyle: "italic", color: "var(--color-gold)" }}>Blog.</em>
          </h1>
        ...
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          <Link href={`/blog/${featured.slug}`} style={{ textDecoration: "none", color: "inherit" }}>
            <div className="blog-featured" style={{ display: "grid", gridTemplateColumns: "1.2fr 1fr", gap: 0, background: "#fff", border: "1px solid rgba(201,149,43,0.1)", borderRadius: 2, overflow: "hidden", transition: "all 0.4s cubic-bezier(0.25,0.46,0.45,0.94)", cursor: "pointer" }}
              onMouseEnter={(e) => { e.currentTarget.style.boxShadow = "0 16px 48px rgba(201,149,43,0.1)"; e.currentTarget.style.borderColor = "rgba(201,149,43,0.2)"; }}
              onMouseLeave={(e) => { e.currentTarget.style.boxShadow = "none"; e.currentTarget.style.borderColor = "rgba(201,149,43,0.1)"; }}
            >
              <div style={{ position: "relative", minHeight: 340, overflow: "hidden" }}>
                <Image src={featured.image} alt={featured.title} fill style={{ objectFit: "cover" }} sizes="60vw" />
                <div style={{ position: "absolute", top: 20, left: 20, background: "var(--color-gold)", color: "var(--color-deep)", fontFamily: "var(--font-body)", fontSize: "0.6rem", fontWeight: 700, letterSpacing: "1.5px", textTransform: "uppercase", padding: "5px 14px", borderRadius: 2 }}>Featured</div>
              </div>
              <div style={{ padding: "48px 40px", display: "flex", flexDirection: "column", justifyContent: "center" }}>
                <div style={{ fontFamily: "var(--font-body)", fontSize: "0.6rem", fontWeight: 700, letterSpacing: "2px", textTransform: "uppercase", color: "var(--color-gold)", marginBottom: 14 }}>{featured.category}</div>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  ...
                  <span style={{ fontFamily: "var(--font-body)", fontSize: "0.72rem", fontWeight: 700, letterSpacing: "1.5px", textTransform: "uppercase", color: "var(--color-gold)" }}>Read Article &rarr;</span>
                </div>
              </div>
            </div>
          </Link>
              ...
              {articles.map((article, i) => (
                <Link href={`/blog/${article.slug}`} key={i} style={{ textDecoration: "none", color: "inherit" }}>
                  <div className="blog-card" style={{ background: "#fff", border: "1px solid rgba(201,149,43,0.08)", borderRadius: 2, overflow: "hidden", transition: "all 0.4s cubic-bezier(0.25,0.46,0.45,0.94)", height: "100%", position: "relative" }}
                    onMouseEnter={(e) => { const el = e.currentTarget; el.style.transform = "translateY(-6px)"; el.style.boxShadow = "0 16px 48px rgba(201,149,43,0.12)"; el.style.borderColor = "rgba(201,149,43,0.25)"; }}
                    onMouseLeave={(e) => { const el = e.currentTarget; el.style.transform = "translateY(0)"; el.style.boxShadow = "none"; el.style.borderColor = "rgba(201,149,43,0.08)"; }}
                  >
                    <div style={{ position: "relative", height: 200, overflow: "hidden" }}>
                      <Image src={article.image} alt={article.title} fill style={{ objectFit: "cover" }} sizes="33vw" />
                      <div style={{ position: "absolute", inset: 0, background: "linear-gradient(180deg, transparent 50%, rgba(26,16,40,0.3) 100%)" }} />
                      <div style={{ position: "absolute", top: 14, left: 14, background: "var(--color-gold)", color: "var(--color-deep)", fontFamily: "var(--font-body)", fontSize: "0.58rem", fontWeight: 700, letterSpacing: "1.5px", textTransform: "uppercase", padding: "4px 12px", borderRadius: 2 }}>{article.category}</div>
                    </div>
                    <div style={{ padding: "24px 24px 28px" }}>
                      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 10 }}>
                        <span style={{ fontFamily: "var(--font-body)", fontSize: "0.68rem", color: "var(--color-body-light)" }}>{article.date}</span>
                        {article.readTime && <span style={{ fontFamily: "var(--font-body)", fontSize: "0.68rem", color: "var(--color-gold)", fontWeight: 600 }}>{article.readTime}</span>}
                      </div>
                      ...
                      <span style={{ fontFamily: "var(--font-body)", fontSize: "0.68rem", fontWeight: 700, letterSpacing: "1.5px", textTransform: "uppercase", color: "var(--color-gold)" }}>Read Article &rarr;</span>
                    </div>
                  </div>
                </Link>
              ))}
      ...
      <section style={{ padding: "80px 56px", background: "linear-gradient(180deg, var(--color-off-white) 0%, #241838 100%)", textAlign: "center", position: "relative", overflow: "hidden" }}>
        <div className="blog-newsletter" style={{ maxWidth: 560, margin: "0 auto", position: "relative", zIndex: 2 }}>
          ...
          <h2 style={{ fontFamily: "var(--font-display)", fontSize: "2.2rem", fontWeight: 400, color: "#fff", marginBottom: 16, lineHeight: 1.2 }}>
            Get Blog Posts <em style={{ fontStyle: "italic", color: "var(--color-gold)" }}>Delivered.</em>
          </h2>
          ...
          <Link href="/#contact" style={{ display: "inline-block", background: "transparent", color: "var(--color-gold)", padding: "12px 36px", fontFamily: "var(--font-body)", fontSize: "0.72rem", fontWeight: 700, textDecoration: "none", letterSpacing: "1.5px", textTransform: "uppercase", borderRadius: 2, border: "1px solid rgba(201,149,43,0.3)" }}>
