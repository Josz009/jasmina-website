...
export default async function EntrepreneurCoaching() {
  const raw = await getServiceBySlug("entrepreneur-coaching").catch(() => null);

  const heroImage = raw?.heroImage ? urlFor(raw.heroImage).width(1400).url() : FALLBACK.heroImage;
  const detailImage = raw?.detailImage ? urlFor(raw.detailImage).width(800).url() : FALLBACK.detailImage;

  return (
    <ServicePage
      title={raw?.title || FALLBACK.title}
      titleAccent={raw?.titleAccent || FALLBACK.titleAccent}
      subtitle={raw?.subtitle || FALLBACK.subtitle}
      heroImage={heroImage}
      intro={raw?.intro || FALLBACK.intro}
      detailImage={detailImage}
      benefits={raw?.benefits?.length ? raw.benefits : FALLBACK.benefits}
      process={raw?.processSteps?.length ? raw.processSteps : FALLBACK.process}
      quote={raw?.quote || FALLBACK.quote}
      pricing={raw?.price ? {
        price: raw.price,
        bookingUrl: raw.bookingUrl || FALLBACK.pricing.bookingUrl,
        steps: raw.pricingSteps?.length ? raw.pricingSteps : FALLBACK.pricing.steps,
      } : FALLBACK.pricing}
    />
  );
}
